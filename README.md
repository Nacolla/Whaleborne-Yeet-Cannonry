...
```bash
git clone -b neoforge-1.21.1 https://github.com/Nacolla/Whaleborne-Yeet-Cannonry.git
cd Whaleborne-Yeet-Cannonry
./gradlew build
```

Produces `build/libs/…-neoforge-1.21.1-*.jar`. The NeoForge runtime is Mojmap natively, so the JAR works in both production and ModDevGradle dev environments without a refmap.
...
On NeoForge these resolve against Mojmap names natively. On Forge they're written the same way and translated to SRG via a MixinGradle refmap.
